...
> **DEPRECATED (2026-07-25).** Auto-generated by `verify_thesis_numbers.py`, which
> checks against the retired Chapter 3 CEIR claim (Chow p-value alone, no negative
> controls). The Chow row below is real but misleading without the robust joint-Wald
> test (p ≈ 0.13, does not reject stability) and negative-control results that
> retired the "energy-specific anchor" reading — see `THESIS_SOURCE_OF_TRUTH.md`
> and `CEIR_FINAL_DIAGNOSIS.md`. Chapters 4–5 rows are still accurate.

**Generated:** 2026-07-24 19:18 UTC  
**Verification:** PASS (checks the wrong ground truth for Ch.3 — see note above)  
**Regenerate:** `npm run thesis:verify` or `npm run thesis:figures` (not recommended — see deprecation notes in `verify_thesis_numbers.py`)
