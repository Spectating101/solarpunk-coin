...
def combined_stress_test(
    ...
    for vol in vol_range:
        ...
        for rate in rate_range:
            if method == "binomial":
                tree = BinomialTree(
                    S0=S0, K=K, T=T, r=rate, sigma=vol, N=N, payoff_type=payoff_type
                )
                price = tree.price()
            else:
                mc = MonteCarloSimulator(
                    S0=S0, K=K, T=T, r=rate, sigma=vol, payoff_type=payoff_type
                )
                price = mc.price()
...
def export_to_excel(
    ...
    combined_stress_df: Optional[pd.DataFrame] = None,
) -> None:
    """
...
def portfolio_greeks(
    contracts: List[Dict[str, float]], method: str = "binomial", N: int = 100
) -> Dict[str, float]:
    """
