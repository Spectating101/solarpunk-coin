...
class EnergyDerivativesPlotter:
    ...
    def plot_monte_carlo_distribution(
        ...
        ax.axvline(
            np.mean(sim.terminal_prices),
            color="green",
            linestyle="--",
            linewidth=2,
            label=f"Mean = ${np.mean(sim.terminal_prices):.2f}",
        )
        ...
        ax.set_ylabel("Frequency", fontsize=11, fontweight="bold")
        ax.set_title(
            f"Payoff Distribution ({payoff_type.capitalize()})", fontsize=12, fontweight="bold"
        )
        ax.legend()
        ax.grid(True, alpha=0.3)

        fig.suptitle(
            f"Monte-Carlo Simulation Results ({num_simulations:,} paths)\n"
    ...
    def plot_greeks_curves(
        ...
        greeks_data = {"Price": [], "Delta": [], "Gamma": [], "Vega": [], "Theta": [], "Rho": []}
    ...
    def plot_stress_test_volatility(
        ...
        ax.set_ylabel("Option Price ($)", fontsize=12, fontweight="bold")
        ax.set_title(
            f"Stress Test: Price vs Volatility\n({payoff_type.capitalize()} | "
    ...
    def plot_stress_test_rate(
        ...
        for rate in rates:
            sim = MonteCarloSimulator(
                S0, K, T, rate, sigma, num_simulations, payoff_type=payoff_type
            )
            price = sim.price()
            prices.append(price)
        ...
        ax.set_ylabel("Option Price ($)", fontsize=12, fontweight="bold")
        ax.set_title(
            f"Stress Test: Price vs Interest Rate\n({payoff_type.capitalize()} | "
    ...
    def plot_price_comparison(
        ...
        ax.errorbar(
            1,
            mc_price,
            yerr=[[mc_price - mc_low], [mc_high - mc_price]],
            fmt="none",
            ecolor="black",
            capsize=10,
            capthick=2,
