...
class MonteCarloSimulator:
    ...
    def simulate_paths(
        self, num_steps: int = 252, return_paths: bool = False
    ) -> Optional[np.ndarray]:
        """
    ...
    def confidence_interval(
        self, num_steps: int = 252, confidence: float = 0.95
    ...
    def stress_test(
        self, volatilities: Optional[List[float]] = None, num_steps: int = 252
    ) -> pd.DataFrame:
        """
        ...
        for vol in volatilities:
            ...
            results.append(
                {"Volatility": f"{vol:.1%}", "Price": price, "Change": price - base_price}
            )

        return pd.DataFrame(results)

    def rate_sensitivity(
        self, rates: Optional[List[float]] = None, num_steps: int = 252
    ) -> pd.DataFrame:
        """
        ...
        for rate in rates:
            ...
            price = sim.price(num_steps)
            results.append({"Rate": f"{rate:.1%}", "Price": price})
