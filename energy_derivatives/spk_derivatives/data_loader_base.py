...
class EnergyDataLoader(ABC):
    ...
    def get_volatility_params(
        ...
        if method == "log":
            ...
            raise ValueError(
                f"Unknown method: '{method}'. " f"Choose from: 'log', 'pct_change', 'normalized'"
            )
        ...
        if not np.isfinite(annual_vol) or annual_vol <= 0:
            warnings.warn(
                f"Invalid volatility {annual_vol}; defaulting to 20%. " f"Check input data."
            )
            annual_vol = 0.20
    ...
    def load_parameters(
        ...
        **loader_kwargs
        ...
        sigma, data_with_returns = self.get_volatility_params(
            self.data_df, method=volatility_method, cap_volatility=volatility_cap
        )
