...
def compute_ceir_column(df: pd.DataFrame, electricity_price: float = 0.05) -> pd.DataFrame:
    ...
    df["Daily_Energy_Cost_USD"] = (
        df["Daily_Energy_TWh"] * electricity_price * 1e9
    )  # Convert TWh to kWh
...
def compute_energy_price(
    ceir_df: pd.DataFrame, normalization_date: Optional[str] = None
) -> np.ndarray:
    """
...
def estimate_volatility(price_series: np.ndarray, periods: int = 252) -> float:
    """
...
def load_parameters(
    ...
    # Load CEIR data
    ceir_df = load_ceir_data(
        data_dir, use_repo_fallback=use_repo_fallback, use_live_if_missing=use_live_if_missing
    )
...
def get_ceir_summary(ceir_df: pd.DataFrame) -> Dict:
    ...
    summary = {
        ...
        "energy_twh_mean": (
            ceir_df["Energy_TWh_Annual"].mean() if "Energy_TWh_Annual" in ceir_df else np.nan
        ),
    }
