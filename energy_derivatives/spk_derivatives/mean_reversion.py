...
class OrnsteinUhlenbeck:
    ...
    def __init__(
        self, S0: float, kappa: float, theta: float, sigma: float, T: float, r: float = 0.02
    ):
    ...
    def simulate_paths(
        self, num_paths: int = 10000, num_steps: int = 252, seed: Optional[int] = None
        ...
        if self.kappa > 1e-8:  # Avoid division by zero
            std_increment = self.sigma * np.sqrt(
                (1 - np.exp(-2 * self.kappa * dt)) / (2 * self.kappa)
            )
        else:
        ...
        for i in range(num_steps):
            ...
            paths[i + 1, :] = (
                self.theta + (paths[i, :] - self.theta) * exp_kappa_dt + std_increment * Z
            )
    ...
    def price_european_call(
        self, K: float, num_paths: int = 100000, seed: Optional[int] = None
    ) -> Dict[str, float]:
        """
    ...
    @staticmethod
    def calibrate_from_data(prices: np.ndarray, dt: float = 1 / 252) -> Dict[str, float]:
        """
    ...
    def get_params_summary(self) -> pd.DataFrame:
        ...
        data = {
            ...
            "Value": [self.S0, self.kappa, self.theta, self.sigma, self.T, self.r, half_life * 252],
            "Description": [
                "Initial price",
...
def compare_gbm_vs_ou(
    ...
    for i in range(num_steps):
        Z = rng.standard_normal(num_paths)
        gbm_paths[i + 1, :] = gbm_paths[i, :] * np.exp(
