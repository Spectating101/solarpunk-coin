...
class BinomialTree:
    ...
    def sensitivity_analysis_convergence(self, step_range: List[int] = None) -> pd.DataFrame:
        """
        ...
        results = []
        for N in step_range:
            tree = BinomialTree(self.S0, self.K, self.T, self.r, self.sigma, N, self.payoff_type)
            price = tree.price()
...
def price_energy_call(S0: float, K: float, T: float, r: float, sigma: float, N: int = 100) -> float:
    """
...
def price_energy_claim(S0: float, T: float, r: float, sigma: float, N: int = 100) -> float:
    """
