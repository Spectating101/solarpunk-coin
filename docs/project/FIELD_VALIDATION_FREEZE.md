...
**Enforcement:** active on `main`  
**Field validation:** deferred by the project owner until the flagship product-polish pass is complete  
**Default rule:** no speculative functionality that expands the research claim, custody model, backend, token, or chain surface
...
3. progressive disclosure that preserves full desktop analysis;
...
3. The bounded v0.3 product-polish exception above  
4. Changes required to ingest or verify a real external source (Issue #3 / Gates 1B–1D) when field validation resumes  
5. Minimal documentation or release metadata required to execute and audit either the polish or field-validation program
...
- Changes that hide evidence boundaries or collapse declared, modeled, observed, and derived objects
