...
Do not apply everywhere with the same story. SolarPunk has three credible grant angles:

1. **Ethereum public-good infrastructure:** open-source energy-risk settlement primitives, live Sepolia proof, audit/readiness path.
2. **Chainlink oracle vertical:** NASA POWER + energy-market data as a new oracle category, Automation/Functions migration, custom data-feed opportunity.
3. **Audit/L2 pilot support:** security audit and capped L2 deployment once the chain lane is selected.
...
- `docs/grants/APPLICATION_PACKET_CHECKLIST.md`
- `docs/grants/DEMO_WALKTHROUGH_SCRIPT.md`
...
- Use the live margin wording: current Sepolia is 150% / 75%; 250% / 125% is the next risk-boxed pilot target.
...
1. Send advisor/thesis-supervisor acknowledgement request.
2. Send 5-10 energy/operator discovery emails.
3. Record the walkthrough video.

### Week 1

1. Follow up on ESP/Chainlink if no confirmation email arrives.
2. Create a public demo URL.
3. Prepare Arbitrum audit lane only if Arbitrum is chosen as production L2.

## Decision Rule

Continue aggressively if any two of these occur:

- Grant office hours or serious reviewer response.
- Chainlink BUILD interview / ecosystem response.
- Advisor or academic acknowledgement.
- Solar/energy stakeholder conversation.
