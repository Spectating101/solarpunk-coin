...
function selectActualGenerationRows(rows, customer, days) {
  ...
  if (actual.length < days) {
    throw new Error(`requested ${days} actual GG days but only ${actual.length} are available`);
  }
  return actual.slice(0, days);
}

function buildHalfHourGenericCsv(selected, timeColumns, customer) {
  ...
  for (const { row: gg, date } of selected) {
    const matching = selected.__allRows.filter((candidate) => (
    ...
    for (const label of timeColumns) {
      ...
      sourceRows.push({ date, interval_end_label: label, gg_kwh: generation, gc_kwh: generalLoad, cl_kwh: controlledLoad, derived_surplus_kwh: derivedExport });
...
async function main() {
  ...
  if (!csvPath || !archivePath) throw new Error('--csv and --archive are required');
  ...
  if (archiveSha256 !== EXPECTED_ARCHIVE_SHA256) {
    throw new Error(`mirror archive SHA-256 mismatch: ${archiveSha256}`);
  }
  if (archiveBytes !== EXPECTED_ARCHIVE_BYTES) {
    throw new Error(`mirror archive byte-length mismatch: ${archiveBytes}`);
  }

  const rawCsv = await readFile(csvPath, 'utf8');
  const trimmed = trimToAusgridHeader(rawCsv);
  const { rows } = parseCsv(trimmed);
  const firstRow = rows[0] || {};
  const timeColumns = Object.keys(firstRow).filter((key) => /^\d{1,2}:\d{2}$/.test(key));
  ...
  Object.defineProperty(selected, '__allRows', { value: rows, enumerable: false });
  const { csv: normalizedCsv, sourceRows } = buildHalfHourGenericCsv(selected, timeColumns, customer);
  ...
  const firstStart = evidence.intervals[0].window_start;
  const lastEnd = evidence.intervals[evidence.intervals.length - 1].window_end;
  const caseManifest = {
    ...
    spatial_identity: null,
    measurement_window: { start: firstStart, end: lastEnd },
  ...
  if (open.decision.decision === 'ADMIT_WITH_LIMIT') {
    ...
    receipt = buildDecisionReceipt({
      ...
      runtime: { package: '@solarpunk/constraint-core', package_version: '0.1.0-alpha.1', source_revision: SOURCE_REVISION },
    ...
    const run = { caseManifest, evidence, contexts: [context], policy: open.policy, scenario, provenance, decision: open.decision, receipt };
    capsule = await buildResearchCapsule(run, receipt);
  ...
  const report = {
    ...
    transformations: {
      ...
      manual_transformations: ['reshape 48 half-hour columns to interval rows', 'combine GC + CL as site_load_kwh', 'derive conservative interval surplus'],
      unresolved: ['mirror bytes not independently matched to historical Ausgrid-hosted bytes', 'no source-holder confirmation', 'derived surplus is not directly metered grid export'],
    },
    evidence: {
