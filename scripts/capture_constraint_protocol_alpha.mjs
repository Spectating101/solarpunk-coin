...
async function openBrief(page) {
  ...
  await page.getByRole('heading', { name: /What did the stricter rule buy—and where did it still fail/i }).waitFor();
}

async function openStudy(page) {
  await page.goto('http://127.0.0.1:4173/#study', { waitUntil: 'networkidle' });
...
await shot(desktop.page, '03-empirical-study.png');
...
await shot(desktop.page, '04-policy-frontier.png');
...
await shot(desktop.page, '05-stress-replay.png');
...
await shot(desktop.page, '06-methods-dossier.png');
await openReproduce(desktop.page);
await shot(desktop.page, '07-reproduction-receipt.png');

await openProtocol(desktop.page);
await shot(desktop.page, '08-protocol-entry.png');
await runSample(desktop.page);
await shot(desktop.page, '09-cumulative-evidence.png');
...
await shot(desktop.page, '10-claim-admitted.png');
...
await shot(desktop.page, '11-settlement-shortfall.png');
await runSample(desktop.page, 'Utility / Green Button');
await shot(desktop.page, '12-utility-evidence.png');
...
await shot(desktop.page, '13-signed-attestation.png');
...
await openBrief(mobile.page);
await shot(mobile.page, '14-mobile-decision-brief.png');
await openStudy(mobile.page);
await shot(mobile.page, '15-mobile-empirical-study.png');
...
await shot(mobile.page, '16-mobile-stress-replay.png');
await openReproduce(mobile.page);
await shot(mobile.page, '17-mobile-reproduction.png');
await openProtocol(mobile.page);
await shot(mobile.page, '18-mobile-protocol-entry.png');
await runSample(mobile.page);
await shot(mobile.page, '19-mobile-evidence.png');
