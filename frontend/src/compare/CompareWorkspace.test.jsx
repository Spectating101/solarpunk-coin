...
describe('CompareWorkspace', () => {
  it('renders a policy manifest diff and 5 × 3 decision matrix under one assurance scenario', async () => {
    renderCompare();

    expect(screen.getByRole('heading', { name: /where do policies disagree/i })).toBeInTheDocument();
    expect(screen.getByText(/15 deterministic decisions are evaluated from 5 committed cases and 3 V2 policies/i)).toBeInTheDocument();
    expect(screen.getByText('5 cases × 3 policies')).toBeInTheDocument();
    ...
    expect(within(table).getByText('CPT-001')).toBeInTheDocument();
    expect(within(table).getAllByRole('button')).toHaveLength(15);
  });
